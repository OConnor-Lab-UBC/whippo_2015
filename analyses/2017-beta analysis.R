...
# compress to summary per sitetime for global analysis

beta_sim_data <- new.comm %>% 
  select(-Sample) %>%
  group_by(sitetime) %>%
  summarise_each(funs(sum))
beta_sim_data <- data.frame(beta_sim_data)

#SKIP AHEAD FROM HERE FOR TRADITIONAL BETA CALCS

# Raup-Crick metrics here:
